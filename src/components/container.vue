...
export default {
  ...
  methods: {
    ...
    //滚动事件
    listscroll() {
      if (this.getScrollTop() + this.getWindowHeight() == this.getScrollHeight() && this.song_list.length<=100){
        let len = this.song_list.length;
        let url = 'api/v1/restserver/ting?method=baidu.ting.billboard.billList&type=1&size=20&offset=' + len
        this.$axios.get(url).then(
          (res) => {
            // let len=this.song_list.length;
            for (let key in res.data) {
              if (key == 'song_list') {
                let songs = res.data[key];
...
.songlistTop>div:nth-child(1),
.songlist_container>div:nth-child(1) {
  width: 5%;
}

.songlistTop>div:nth-child(2),
.songlist_container>div:nth-child(2) {
  width: 25%;
}

.songlistTop>div:nth-child(3),
.songlist_container>div:nth-child(3) {
  width: 25%;
}

.songlistTop>div:nth-child(4),
.songlist_container>div:nth-child(4) {
  width: 25%;
}

.songlistTop>div:nth-child(5),
.songlist_container>div:nth-child(5) {
  width: 20%;
}
